...
- README: new "Getting `PROVABLY_API_KEY` and `PROVABLY_ORG_ID`" subsection walking through sign-up at app.provably.ai → create org → User settings → API Key → Active key, plus a pointer to provably.ai/docs.
...
- Dev tooling: added a `pytest-cov` coverage gate (`pytest-cov>=5` dev extra). CI and the Docker `test` stage now enforce a 60% floor on the hermetic **unit** suite (`pytest tests/unit --cov=provably --cov-fail-under=60`); the threshold lives in `[tool.coverage.report]` and coverage stays out of default `pytest` options so the inner loop is unchanged. Expanded unit tests for previously thin modules (`handoff/_resources`, `handoff/_discovery`, `handoff/client`, `handoff/_http`, `intercept/_loader`); unit coverage is now ~72%. Docker `test` stage now installs `pytest-cov` alongside other dev tools (fixes coverage-gated CMD). Ruff `exclude` lists `.cache`/`.venv` so `ruff check` matches CI on machines with local pre-commit caches.
